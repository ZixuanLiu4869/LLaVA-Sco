...
def main(args):
    ...
    if args.gradio_ui:
        ...
        #test_dataset = load_dataset("Lin-Chen/MMStar", split="val")
        #test_dataset = load_dataset("lmms-lab/ai2d", split="test")
        #test_dataset = load_dataset("HuggingFaceM4/MMBench_dev", split="train")
        ...
        for data in test_dataset:
            #print(data["image"])
            image_path = "../../mathvista/"+data['image']
            image = process_image(image_path=image_path)
            #image = data["image"]
            #image = process_image(image_path=image_dir+"/"+image_path)
            #prompt_text = data['question'] + "   Options: "+",".join(data["options"])
            #options_content = []
            #for opt in options:
                #opt_content = data[opt]
                #if opt_content is not None:
                    #options_content.append(f"{opt}: {opt_content}")

            #prompt_text = data["question"] + " Options:" + f"{' '.join(options_content)}"
            prompt_text = data["question"]
            
            #print(prompt_text)
            result = generate_text_from_image(
                model, processor, image, prompt_text, args.temperature, args.top_p
            )
            print("Generated Text:", result)
            
            #ground_truth_index = int(data["answer"])
            #ground_truth = data["options"][ground_truth_index]

            #ground_truth_index = int(data["label"])
            #correct_option = options[ground_truth_index]
            #correct_content = data[correct_option]
            #ground_truth = f"{correct_option}: {correct_content}"

            #print("Ground truth: ", ground_truth)
            ground_truth = data["answer"]
            result_list.append({"prompt": prompt_text, "answer": result, "ground_truth": ground_truth})


    with open("answer_for_detectiumfire_sft.json", "w") as f:
